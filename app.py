...
def beregn_poeng(mål_hjemme, mål_borte, resultat, res):
    poeng = 0
    if resultat == res['resultat']: poeng += 2
    if int(mål_hjemme) == res['mål_hjemme']: poeng += 1
    if int(mål_borte) == res['mål_borte']: poeng += 1
...
def fasit():
    ...
    for kid, hjemmelag, bortelag, mål_hjemme, mål_borte, resultat, dato in resultater_data:
        fasit_liste.append({'kamp_id': kid, 'hjemmelag': hjemmelag, 'bortelag': bortelag,
...
def dagsvinner():
    ...
    for dato in datoer:
        ...
        if not dagens_kamper: continue
        c.execute('SELECT navn, telefon, epost, kamp_id, mål_hjemme, mål_borte, resultat FROM tips')
        bruker_poeng = {}
        for navn, telefon, epost, kid, mål_hjemme, mål_borte, resultat in c.fetchall():
            if kid in dagens_kamper:
...
def administrer():
    ...
    if request.method == 'POST':
        ...
        if action == 'slett_tips':
            navn = request.form.get('navn'); telefon = request.form.get('telefon'); epost = request.form.get('epost')
            ...
            c.execute('DELETE FROM tips'); c.execute('DELETE FROM gruppetips'); c.execute('DELETE FROM sluttspilltips')
            conn.commit(); melding = "✅ Alle tips er slettet!"
...
def resultater():
    ...
    for gruppe, lag, plassering in c.fetchall():
        if gruppe not in eksisterende_gruppefasit: eksisterende_gruppefasit[gruppe] = {}
        eksisterende_gruppefasit[gruppe][lag] = plassering
    c.execute('SELECT fase, lag FROM sluttspillfasit')
    eksisterende_sluttspillfasit = {}
    for fase, lag in c.fetchall():
        if fase not in eksisterende_sluttspillfasit: eksisterende_sluttspillfasit[fase] = []
...
def send_email(rangering):
    sender = SMTP_AVSENDER; password = SMTP_PASSORD
    ...
    body += '</table>'
    msg = MIMEMultipart()
    msg['From'] = sender; msg['Subject'] = 'Oppdatert poengoversikt - VM Tipping'
    ...
    try:
        with smtplib.SMTP_SSL('smtp.gmail.com', 465, timeout=10) as server:
            server.login(sender, password); server.sendmail(sender, recipients, msg.as_string())
        return True
    except:
        try:
            with smtplib.SMTP('smtp.gmail.com', 587, timeout=10) as server:
                server.starttls(); server.login(sender, password); server.sendmail(sender, recipients, msg.as_string())
            return True
        except:
            return False
